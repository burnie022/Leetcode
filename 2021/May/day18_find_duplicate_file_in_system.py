...
class Solution:
    def findDuplicate(self, paths: List[str]) -> List[List[str]]:
        content_file_dict = {}
        duplicate_content_dict = {}

        for path_info in paths:
            path_info_split = path_info.split()
            path, file_info = path_info_split[0], path_info_split[1:]
            for f in file_info:
                file, content = f[:len(f)-1].split("(")
                content_hash = hash(content)
                if content_hash in content_file_dict:
                    if content_hash not in duplicate_content_dict:
                        duplicate_content_dict[content_hash] = [content_file_dict[content_hash]]
                    duplicate_content_dict[content_hash].append(path + "/" + file)
                else:
                    content_file_dict[content_hash] = path + "/" + file

        return list(duplicate_content_dict.values())
...
tests = [
    ["root/a 1.txt(abcdefghijklmnop) 2.txt(efgh)","root/c 3.txt(abcdefghijklmnop)","root/c/d 4.txt(efgh)",
        "root 4.txt(efgh)"],
    ["root/a 1.txt(abcd) 2.txt(efgh)","root/c 3.txt(abcd)","root/c/d 4.txt(efgh)"]
]

for t in tests:
    print(obj.findDuplicate(t), end="\n\n")

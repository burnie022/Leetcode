...
Hint #3
    ...
    to √n because, if n is divisible by some number p, then n = p × q and since p ≤ q, we could derive that p ≤ √n.
...
Hint #4
    VIEW PIC: count_primes_Sieve_of_Eratosthenes_animation.gif
...
Hint #5
    ...
    5 × 2 = 10, 5 × 3 = 15, 5 × 4 = 20, 5 × 5 = 25, ... can be marked off. There is a slight optimization here, we do
    not need to start from 5 × 2 = 10. Where should we start marking off?
Hint #6
...
Hint #7
    It is easy to say that the terminating loop condition is p < n, which is certainly correct but not efficient. Do
    you still remember Hint #3?
Hint #8
    Yes, the terminating loop condition can be p < √n, as all non-primes ≥ √n must have already been marked off. When
...
class Solution:
    ...
    #                 nums[j] = 1
    #
    # return n - sum(nums) - 2 if n > 2 else 0

    # def countPrimes(self, n: int) -> int:
    #     primes = 1 if n > 2 else 0
    #     nums = [0] * n
...
tests = [
    0, 1, 2, 3, 4, 5, 10, 25, 100, 96, 109, 114, 290, 990, 1300, 500000, 1000000, 5000000
]

for t in tests:
    print(t)
    print(obj.countPrimes(t), end="\n\n")

# print(obj.countPrimes(25), end="\n\n")

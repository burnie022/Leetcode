...
class Solution:
    def maximumUnits(self, boxTypes: List[List[int]], truckSize: int) -> int:
        boxTypes = deque(sorted(boxTypes, key=lambda x: x[1], reverse=True))
        boxes = units = 0

        while boxTypes and boxes < truckSize:
            if boxTypes[0][0] < truckSize - boxes:
                boxes += boxTypes[0][0]
                units += boxTypes[0][1] * boxTypes[0][0]
                boxTypes.popleft()
            else:
                units += boxTypes[0][1] * (truckSize - boxes)
                break
